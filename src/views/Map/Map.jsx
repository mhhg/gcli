...
const MAP_API = "https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=geometry,drawing,places";

let markers = [];
let MapComponent = compose(withProps({
  ...
  scaleControl={false} defaultOptions={{
    scrollwheel: true
  ...
  // mapTypeId={"ROADMAP"}
  zoom={13} defaultCenter={{ lat: 35.7412568, lng: 51.4368788 }}>
  <MarkerClusterer onClick={props.onMarkerClustererClick} averageCenter
    enableRetinaIcons gridSize={60}>
    {props.markers.map(marker => (<Marker
      key={marker.id} position={{ lat: marker.latitude, lng: marker.longitude }}
    />))}
  </MarkerClusterer>
  {/* {props.isMarkerShown && (
...
class Map extends Component {
  constructor(props) {
    super(props);
    const bounds = { sw: { lat: -60, lng: -90 }, ne: { lat: 60, lng: -90 } };
    // const mapPaneName = OverlayView.OVERLAY_MOUSE_TARGET;
    const initialFilter = { logic: "and", filters: [] };
    this.defaultPageSize = 20000; // set some constants
    ...
    Socket.emit("provider:read", JSON.stringify({
      skip: 0, limit: this.defaultPageSize, search: { ownership: 0 }, filter: initialFilter,
      sort: []
    }), this.callbackRead);
    this.state = {
      dropdownOpen: false, radioSelected: 2,
      isMarkerShown: false, events: this.logs
    };
  ...
  callbackRead(stringResponse) {
    ...
    console.log("[Provider.funcResponseCallback] state.markers:", response.providers);
  }
  toggle() {
  ...
  render() {
    return (
      <div className="animated fadeIn">
        <this.MyMapComponent markers={this.state.markers}
