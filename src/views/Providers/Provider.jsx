...
class Provider extends React.Component {
    constructor(props) {
        ...
        Socket.emit('provider:read', JSON.stringify({ // emit initial request
            skip: 0, limit: 40, search: { ownership: 2 }, filter: initialFilter, sort: []
        }), this.callbackRead);
        this.state = {
            allowUnsort: true, multiple: false, pagerState: {
                info: true, type: 'numeric', pageSizes: [5, 10, 25, 50, 75, 100],
                previousNext: true, pageSize: this.defaultPageSize, buttonCount: this.defaultButtonCount
            }, filter: initialFilter, search: { ownership: 2 }, sort: [], docInEdit: undefined
        };
    }
    ...
    dialogTitle() {
        return `${this.state.docInEdit.id === undefined ? 'Add' : 'Edit'} provider`;
    }
    ...
    newProvider(source) {
        const doc = {
            imageId: '', ownerId: '', isConfirmed: false,
            latitude: 0.0, longitude: 0.0, name: '', description: '', mobile: '', background: '',
            categories: [], phoneNumbers: [], membersCount: 0, advertisement: '', attachments: []
        };
    ...
    updateOwnerState(key, value) {
        ...
        this.setState({ search: Object.assign(this.state.search, { ownership: parseInt(value, 10) }) });
        ...
        Socket.emit('provider:read', JSON.stringify({
            ...
            search: this.state.search, filter: this.state.filter, sort: this.state.sort
        }), this.callbackRead);
    }
    onPageChange(e) {
    ...
    updatePagerState(key, value) {
        ...
        this.setState(Object.assign({}, this.state, { pagerState: newPagerState }));
    }
    toggle() {
    ...
    filterChange(event) {
        ...
        Socket.emit('provider:read', JSON.stringify({
            ...
            search: this.state.search, filter: event.filter, sort: this.state.sort
        }), this.callbackRead);
    }
    sortChange(event) {
    ...
    render() {
        const cellWithEditing = function (edit, remove) {
          ...
          <button className="k-button k-grid-remove-command" onClick={() => {
        ...
        const grid = (<Grid data={this.state.data} style={{ maxHeight: '750px' }}
            ...
            <Column field="phoneNumbers" title="PhoneNumbers" editable={false} width="200px" />
            <Column field="mobile" title="Mobile" width="200px" />
            <Column field="membersCount" title="Members No" editable={false} width="200px" />
            <Column field="background" title="Background" editable={false} sortable={false} width="200px" />
        ...
        const dialog = this.state.docInEdit && (<Dialog title={this.dialogTitle()}
            ...
            <form className="row k-form" onSubmit={this.handleSubmit}>
                <div style={{ marginBottom: '1rem' }} className="col-md-6"><label>Name<br />
                    <Input className="form-control" type="text" name="name"
                        value={this.state.docInEdit.name || ''} onChange={this.onDialogInputChange} /></label>
                </div>
                <div style={{ marginBottom: '1rem' }} className="col-md-6"><label>Categories<br />
                ...
                <div style={{ marginBottom: '1rem' }} className="col-md-6"><label>Mobile<br />
                    <Input className="form-control" type="text" name="mobile"
                        value={this.state.docInEdit.mobile || ''} onChange={this.onDialogInputChange} /></label>
                </div>
                <div style={{ marginBottom: '1rem' }} className="col-md-6"><label>Background<br />
          ...
          value={this.state.docInEdit.longitude || ''} onChange={this.onDialogInputChange} /></label>
                ...
                <div style={{ marginBottom: '1rem' }} className="col-md-6"><label>Longitude<br />
                    <Input className="form-control" type="number" name="longitude"
                        value={this.state.docInEdit.longitude || ''} onChange={this.onDialogInputChange} /></label>
                </div>
        ...
        const ownershipState = (<dl><dt>Ownership status filter:</dt><dd>
            ...
            <input defaultChecked={true} type="radio" name="ownership" id="ownershipRegistered"
                className="k-radio" value="2" onChange={e => { this.updateOwnerState('type', e.target.value); }} />
            <label style={{ margin: '7px 3em 7px 0px', lineHeight: '1.2' }}
                className="k-radio-label" htmlFor="ownershipRegistered">Registered&nbsp;</label>
            <input type="radio" name="ownership" id="ownershipAll" className="k-radio" value="0"
        ...
        const filterState = (<dl><dt>Filter state:</dt><dd>
            <input type="checkbox" className="k-checkbox" id="unsort"
            ...
            <label htmlFor="unsort" className="k-checkbox-label"
                style={{ lineHeight: '1.2', marginBottom: '1em' }}>Enable UnSorting</label><br />
            ...
            <label htmlFor="multiSort" className="k-checkbox-label" style={{ lineHeight: '1.2' }}>Enable multiple columns sorting</label>
        </dd></dl>
        );
        const pagerTypeState = (<dl><dt>Pager type:</dt><dd>
            <input type="radio" name="pager" id="numeric" className="k-radio" value="numeric"
                defaultChecked={true} onChange={e => { this.updatePagerState('type', e.target.value) }} />
            <label style={{ margin: '7px 3em 7px 0px', lineHeight: '1.2' }}
        ...
        const buttonsState = (<dl><dt>Max. number of buttons:</dt>
            <dd><input defaultValue="5" className="k-textbox" type="number"
                onChange={e => { this.updatePagerState('buttonCount', e.target.valueAsNumber); }} />
            </dd></dl>
        );
        const cardHeader = (
            <CardHeader>Providers Grid State Form
        <div className="card-header-actions">
                    <a className="card-header-action btn btn-minimize" data-target="#collapseExample"
                        onClick={this.toggle}><i className="icon-arrow-up" /></a>
                </div>
            </CardHeader>
        );
        ...
        return (<div>
            {columnCard}
            {grid}
            <Row><Col md="12" style={{ padding: '0px', marginBottom: '5px' }}>{dialog}</Col></Row>
        </div>);
    }
}
